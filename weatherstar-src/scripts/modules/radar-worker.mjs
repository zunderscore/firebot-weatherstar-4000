...
import radarBlob from '../../images/maps/radar.webp';

const radarFullSize = { width: 2550, height: 1600 };
const radarFinalSize = { width: 640, height: 367 };

const getBlobFromDataURL = (url) => {
	const blobParts = url.split(',');

	const byteString = atob(blobParts[1]);
	const mimeType = blobParts[0].split(':')[1].split(';')[0];
...
const baseMapImages = new Promise((resolve) => {
	createImageBitmap(getBlobFromDataURL(radarBlob)).then((imageBitmap) => {
...
onmessage = async (e) => {
	const {
		url, radarSourceXY, sourceXY, offsetX, offsetY,
	} = e.data;
